...
Discovery đã được Project Owner khởi động ngày `2026-09-08`; scaffold Gate 0 đã được tạo và quality gate pass. Đây chưa phải cam kết delivery hoặc quyền product build. Với extensibility kernel, bốn lớp phân quyền và self-service có giới hạn cho setting/form/workflow trong P0, ROM sơ bộ là `28–32 tuần ±30%` với 7 core FTE cộng specialist part-time. Phương án `26–29 tuần ±30%` cần bổ sung 1 Platform/Backend, 1 Frontend/Admin UX và 0,5 QA/Security Automation; chỉ có thể giữ `24–26 tuần` nếu chuyển các editor self-service form/workflow sang P1. Estimate được re-baseline sau gate Go/Adjust/Stop.

...
| `G0-06` — synthetic environment | Blocked / 15%; D-025 image policy approved, digest/service/isolation còn thiếu | `Forge` | `Forge` | `Sentinel` + `Beacon` | Gate 0 | [Image policy](../infra/local/IMAGE-POLICY.md), [environment preflight](evidence/G0-004-G0-006-PREFLIGHT.md); resolved digest, services và isolation evidence còn thiếu |

...
| `G0-06` — synthetic environment | Blocked / 30%; Compose/fake-service scaffold validated, digest/runtime/isolation còn thiếu | `Forge` | `Forge` | `Sentinel` + `Beacon` | Gate 0 | [Image policy](../infra/local/IMAGE-POLICY.md), [Compose scaffold](../infra/local/compose.synthetic.yml), [environment preflight](evidence/G0-004-G0-006-PREFLIGHT.md); resolved digest, runtime và isolation evidence còn thiếu |

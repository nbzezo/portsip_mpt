...
| `G0-06` — synthetic environment | Blocked / 45%; runtime replay healthy and image digests captured, reviewer/isolation sign-off còn thiếu | `Forge` | `Forge` | `Sentinel` + `Beacon` | Gate 0 | [Image policy](../infra/local/IMAGE-POLICY.md), [Compose scaffold](../infra/local/compose.synthetic.yml), [environment preflight](evidence/G0-004-G0-006-PREFLIGHT.md); reviewer/isolation evidence còn thiếu |

...
| D-025 | Synthetic image/version/checksum policy | Official PostgreSQL `18.0-bookworm` và Redis `8.2.2-bookworm`, tag + immutable digest; fake IdP/PortSIP/CRM do repository sở hữu; dedicated network và synthetic-only data | **Approved policy / digest captured; review pending** | `Owner` | 2026-09-11 | [Image policy](../infra/local/IMAGE-POLICY.md), [G0-06 review](evidence/G0-06-REVIEW.md); `Sentinel`/`Beacon` sign-off còn thiếu |

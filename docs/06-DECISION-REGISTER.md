...
| D-010 | Build authorization | **Adjust direction selected**: giữ kernel + role/config templates và voice P0; chuyển visual/self-service form/workflow builders sang P1; trunk write có thể dời wave sau | **Adjust selected / conditional — build not authorized** | `Owner` | 2026-09-09 direction; final gate after evidence | [Adjust evidence](evidence/D-010-ADJUST.md); cần Gate 0, exact PortSIP/PoC, security/operations evidence và signed final authorization trước product build |
...
| D-025 | Synthetic image/version/checksum policy | Official PostgreSQL `18.0-bookworm` và Redis `8.2.2-bookworm`, tag + immutable digest; fake IdP/PortSIP/CRM do repository sở hữu; dedicated network và synthetic-only data | **Approved policy / digest capture pending** | `Owner` | 2026-09-11 | [Image policy](../infra/local/IMAGE-POLICY.md); cần Docker daemon ổn định, resolved digest và `Sentinel`/`Beacon` review trước G0-06 acceptance |
